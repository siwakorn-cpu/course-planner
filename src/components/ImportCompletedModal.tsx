// ImportCompletedModal — แสดงผลตรวจสอบไฟล์ Excel หน่วยกิตเดิมก่อนยืนยันนำเข้า
import type { AppDataApi } from '../hooks/useAppData';
import type { CompletedParseResult } from '../importExcel';
import { Modal } from './common/Modal';

interface Props {
  result: CompletedParseResult;
  fileName: string;
  api: AppDataApi;
  onClose: () => void;
...
export function ImportCompletedModal({ result, fileName, api, onClose, onDone }: Props) {
  const confirm = () => {
    api.importCompleted(result.valid.map((r) => r.data!));
    onDone(`นำเข้าหน่วยกิตเดิมสำเร็จ: เพิ่มใหม่ ${result.addCount} · อัปเดต ${result.updateCount} รายการ`);
    onClose();
  };

  return (
    <Modal title={`นำเข้าหน่วยกิตเดิมจาก: ${fileName}`} onClose={onClose}>
      {result.headerError ? (
        ...
        <>
                ...
                {result.rows.map((r) => {
                  ...
                  return (
                    <tr key={r.rowNumber}>
                      ...
                      <td>{r.classLabel ?? <span className="muted">—</span>}</td>
                      <td>{r.data?.code ?? ''}</td>
                      <td>{r.data?.name ?? <span className="muted">{r.errors.join('; ')}</span>}</td>
                      <td className="num">{r.data?.credits ?? ''}</td>
                      <td>
                        {ok ? (
          ...
          {result.invalid.length > 0 && (
            <p className="muted" style={{ fontSize: '0.85rem' }}>
              * แถวที่ผิดพลาดจะถูกข้าม (นำเมาส์ชี้ที่ป้าย “ผิดพลาด” เพื่อดูเหตุผล เช่น ยังไม่มีห้องในระบบ)
            </p>
          )}
        </>
      )}
      ...
      <div className="modal-actions">
        ...
        {!result.headerError && (
          <button className="btn primary" onClick={confirm} disabled={result.valid.length === 0}>
            ยืนยันนำเข้า {result.valid.length} รายการ
          </button>
        )}
      </div>
    </Modal>
