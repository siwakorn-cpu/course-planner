...
export function rowsToSubjects(
  ...
  for (let i = headerIndex + 1; i < aoa.length; i++) {
    ...
    if (!type) errors.push(`ประเภทไม่ถูกต้อง ("${norm(get(row, 'type'))}") ต้องเป็น พื้นฐาน/เพิ่มเติม/กิจกรรมพัฒนาผู้เรียน`);
...
export async function downloadTemplate(): Promise<void> {
  ...
  const help = XLSX.utils.aoa_to_sheet([
    ...
    ['ประเภท', 'ประเภทวิชา', 'พื้นฐาน / เพิ่มเติม / กิจกรรมพัฒนาผู้เรียน'],
...
export async function downloadCompletedTemplate(): Promise<void> {
  ...
  const help = XLSX.utils.aoa_to_sheet([
    ...
    ['ประเภท', 'พื้นฐาน / เพิ่มเติม / กิจกรรมพัฒนาผู้เรียน'],
